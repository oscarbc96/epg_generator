...
def elem_to_programme(elem):
    ...
    for epnumnode in elem.findall('episode-num'):
        if 'episode-num' not in list(d.keys()):
            d['episode-num'] = []
    ...
    for stnode in elem.findall('subtitles'):
        if 'subtitles' not in list(d.keys()):
            d['subtitles'] = []
        std = {}
    ...
    for srnode in elem.findall('star-rating'):
        if 'star-rating' not in list(d.keys()):
            d['star-rating'] = []
        srd = {}
...
class Writer:
    ...
    **All strings passed to this class must be Unicode, except for dictionary
    ...
    def set_zero_ormore(self, programme, element, p):
        ...
        """
        if element in list(programme.keys()):
    ...
    def addProgramme(self, programme):
        ...
        for attr in ('start', 'channel'):
            if attr in list(programme.keys()):
        ...
        # Episode-num
        if 'episode-num' in list(programme.keys()):
        ...
        # Audio details
        if 'audio' in list(programme.keys()):
            a = SubElement(p, 'audio')
            if 'stereo' in list(programme['audio'].keys()):
                s = SubElement(a, 'stereo')
                self.settext(s, programme['audio']['stereo'], with_lang=False)
            if 'present' in list(programme['audio'].keys()):
        ...
        # Previously shown
        if 'previously-shown' in list(programme.keys()):
            ps = SubElement(p, 'previously-shown')
            for attr in ('start', 'channel'):
                if attr in list(programme['previously-shown'].keys()):
    ...
    def addChannel(self, channel):
        ...
        # Icon
        if 'icon' in list(channel.keys()):
            self.seticons(c, channel['icon'])

        # URL
        if 'url' in list(channel.keys()):
